...
-- Height/width of a laid-out frame, taken from its edges. The real 1.12.1
-- client returns GetHeight/GetWidth of a frame sized by its ANCHORS (no
-- SetHeight/SetWidth) multiplied by the UI scale -- measured on
-- LibConfigContentScroll at UI scale 0.64: GetHeight 312.32, GetTop-GetBottom
-- 487.99, and the client's own scroll range 192 = 680 - 488. The edges are
-- right on both clients. Used only for fixed, anchor-sized frames: a frame
-- that was just resized or moved may still report its previous edges.
-- GetHeight/GetWidth is the fallback for a frame not laid out yet.
local function FrameHeight(frame)
    ...
    if okTop and okBottom and tonumber(top) and tonumber(bottom) then
        return top - bottom
    end
...
-- Rebuilds a folder path whose HEAD was cut off, from the installed addons'
-- folder names. The full path is always "Interface\AddOns\<addon>\...", and
-- the surviving tail starts somewhere inside that prefix. An addon qualifies
-- when the end of its "Interface\AddOns\<name>\" overlaps the start of the
-- tail at least up to the tail's first backslash, so the partial first segment
-- is fully accounted for. The longest overlap wins; a tie is ambiguous and
-- returns nil, so the glyphs fall back to text rather than load nothing.
-- Compared case-insensitively, as the legacy client resolves paths that way.
local function RebuildHead(tail)
    if type(GetNumAddOns) ~= "function" or type(GetAddOnInfo) ~= "function" then return nil end
    local lowerTail = string.lower(tail)
    local firstSlash = string.find(lowerTail, "\\", 1, true)
    if not firstSlash then return nil end
    local tailLen = string.len(lowerTail)

    local okCount, count = pcall(GetNumAddOns)
    if not okCount or type(count) ~= "number" then return nil end

    local best, bestOverlap, tie = nil, 0, false
    local i
    for i = 1, count do
        local okInfo, name = pcall(GetAddOnInfo, i)
        if okInfo and type(name) == "string" and name ~= "" then
            local root = "Interface\\AddOns\\" .. name .. "\\"
            local lowerRoot = string.lower(root)
            local rootLen = string.len(lowerRoot)
            local k = (rootLen < tailLen) and rootLen or tailLen
            while k >= firstSlash do
                if string.sub(lowerRoot, rootLen - k + 1) == string.sub(lowerTail, 1, k) then break end
                k = k - 1
            end
            if k >= firstSlash then
                if k > bestOverlap then
                    best, bestOverlap, tie = root .. string.sub(tail, k + 1), k, false
                elseif k == bestOverlap then
                    tie = true
                end
            end
        end
...
local function DeriveOwnPath()
    ...
    if fileStart then
        ...
        -- The real 1.12.1 client (Lua 5.0) truncates a long chunk name from
        -- the FRONT and keeps its end behind a literal "...", confirmed live:
        --
        --   ...vUI_Config\Libraries\LibConfig-1.0\LibConfig-1.0.lua:...
        --
        -- The file name is present, but the folder's head is not, and a path
        -- starting with "..." loads nothing.
        if string.sub(path, 1, 3) == "..." then
            return RebuildHead(string.sub(path, 4))
        end
        return path
    end

    -- CASE 2: the path is truncated from the END, as Unreal Azeroth does.
...
local function CreateDropdown(parent, options)
    ...
    -- measured.) So the anchor crosses the scroll boundary, and the two
    -- clients resolve that differently:
    --
    --  * Real 1.12.1: frames in the scroll child report and anchor at their
    --    SCROLLED, on-screen position (measured: the button's GetBottom
    --    matched the cursor on its edge at scroll offset 288). The plain
    --    anchor is flush; adding the scroll amount put the menu that far
    --    above the button.
    --  * Unreal Azeroth: they report and anchor at their UNSCROLLED position
...
DetectUA = function() -- forward-declared local, assigned here
...
local function CreateSlider(parent, options)
    ...
    local function UsableWidth()
        local trackWidth = FrameWidth(track) or width
...
local function CreateScrollBar(parent, options)
    ...
    local function PlaceThumb(value)
        ...
        local trackHeight = FrameHeight(track) or 0
    ...
    AttachThumbDrag(thumb, ticker, {
        ...
        onUpdate = function()
            ...
            local trackHeight = FrameHeight(track)
            if not (cy and trackTopOk and tonumber(trackTop) and trackHeight) then
                return
            end
...
local function CursorFraction(area)
    ...
    local w = FrameWidth(area)
    local h = FrameHeight(area)
    if not (leftOk and topOk and w and h) then return nil end
    if not (tonumber(left) and tonumber(top)) then
        return nil
    end
...
local function AttachDragMarker(area, marker, onMove)
    ...
    local function ReadFraction()
        ...
        local aW = FrameWidth(area)
        local aH = FrameHeight(area)
...
local function RenderSidebar()
    ...
    local viewHeight = (FrameHeight(sidebar) or 0) - SIDEBAR_PAD * 2
...
-- client).
-- The value handed to `contentScroll:SetVerticalScroll` for a wanted on-screen
-- scroll of `offset`.
--
-- Unreal Azeroth draws a plain ScrollFrame (no nested slider) scrolled by
-- `value + (child height - frame height)` for any NON-ZERO value, and not
-- scrolled at all for exactly 0. Measured on this panel, content 680 / frame
-- 488: 0 shows the page top, 1 the page end, -150 about 42 down; its
-- GetVerticalScrollRange reports the full child height (680). So the wanted
-- offset is shifted down by the overflow, and the one wanted offset that would
-- map to exactly 0 -- the very bottom of the page -- is nudged off it.
