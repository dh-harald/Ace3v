...
Port: WoW 1.12.1 (Lua 5.0) and Unreal Azeroth port of the TBC r68090 library
]]

local MAJOR_VERSION = "LibMobHealth-4.0"
local MINOR_VERSION = 68091
...
-- 1.12.1 has no instance difficulties; every mob is stored as difficulty 1,
-- which keeps the npc[1..3] data layout of the TBC library.
...
-- Event arguments are the arg1..arg4 globals. A handler set to nil is skipped:
-- UnregisterEvent does not stop delivery on every client.
frame:SetScript("OnEvent", function()
	local handler = frame[event]
	if handler then
		handler(lib, arg1, arg2, arg3, arg4)
	end
end)
...
if data then
	...
	elseif not rawget(data.npc, 1) or not rawget(data.npc, 2) or not rawget(data.npc, 3) then
		data.npc = { setmetatable({}, mt), setmetatable({}, mt), setmetatable({}, mt) }
	end
else
	data = {
...
local currentAccumulatedHP = { target = nil }
local currentAccumulatedPercent = { target = nil }
local currentName = { target = nil }
local currentLevel = { target = nil }
local recentDamage = { target = nil }
local lastPercent = { target = nil }

local function trim(text)
	return (string_gsub(text, "^%s*(.-)%s*$", "%1"))
end

-- The saved data lives in the LibMobHealth40DB / LibMobHealth40Opt globals.
-- Whichever addon lists them under ## SavedVariables persists them; the
-- library adopts them at PLAYER_LOGIN, by which time every addon's saved
-- variables are loaded. Without such an addon the data lasts one session.
local function LoadSavedData()
	...
	_G.SlashCmdList["LIBMOBHEALTHFOUR"] = function(text)
		text = trim(string_lower(text or ""))
		local _, _, alpha, bravo = string_find(text, "^([^%s]+)%s+(.*)$")
		if not alpha then
			alpha = text
		end
		if alpha == "" or alpha == "help" then
			DEFAULT_CHAT_FRAME:AddMessage(string_format("|cffffff7f%s|r", MAJOR_VERSION))
			DEFAULT_CHAT_FRAME:AddMessage(string_format(" - |cffffff7f%s|r [%s] - %s", "save", options.save and "|cff00ff00On|r" or "|cffff0000Off|r", "whether to save mob health data"))
			DEFAULT_CHAT_FRAME:AddMessage(string_format(" - |cffffff7f%s|r [%s] - %s", "prune", options.prune == 0 and "|cffff0000Off|r" or "|cff00ff00" .. options.prune .. "|r", "how many data points until data is pruned, 0 means no pruning"))
		elseif alpha == "save" then
			options.save = not options.save
			DEFAULT_CHAT_FRAME:AddMessage(string_format("|cffffff7f%s|r", MAJOR_VERSION))
			DEFAULT_CHAT_FRAME:AddMessage(string_format(" - |cffffff7f%s|r [%s]", "save", options.save and "|cff00ff00On|r" or "|cffff0000Off|r"))
		elseif alpha == "prune" then
			local bravo_num = tonumber(bravo)
			if bravo_num then
				options.prune = math.floor(bravo_num+0.5)
				DEFAULT_CHAT_FRAME:AddMessage(string_format("|cffffff7f%s|r", MAJOR_VERSION))
				DEFAULT_CHAT_FRAME:AddMessage(string_format(" - |cffffff7f%s|r [%s]", "prune", options.prune == 0 and "|cffff0000Off|r" or "|cff00ff00" .. options.prune .. "|r"))
			else
				DEFAULT_CHAT_FRAME:AddMessage(string_format("|cffffff7f%s|r - prune must take a number, %q is not a number", MAJOR_VERSION, bravo or ""))
			end
		else
			DEFAULT_CHAT_FRAME:AddMessage(string_format("|cffffff7f%s|r - unknown command %q", MAJOR_VERSION, alpha))
		end
	end
	...
	-- Drops empty tables and prunes the data down to options.prune. The
	-- metatables are removed first, so indexing a missing level does not
	-- create an empty table; RestoreMetatables puts them back.
	...
	local function RestoreMetatables()
		if not data.legacy then
			data.legacy = {}
		end
		setmetatable(data.npc[1], mt)
	...
	-- Unreal Azeroth writes the saved variables on logout and /reload without
	-- delivering PLAYER_LOGOUT (or PLAYER_LEAVING_WORLD) first, so the same
	-- work also runs here, on the data the previous session saved.
	...
	if type(_G.MobHealth3DB) == "table" then
		for index, value in pairs(_G.MobHealth3DB) do
			if type(index) == "string" and type(value) == "number" then
				local _, _, name, level = string_find(index, "^(.+):(%-?%d+)$")
				if name then
					level = level+0
...
function frame:UNIT_COMBAT(unit, _, _, damage)
	if unit ~= "target" or not currentAccumulatedHP[unit] then
		return
	end
...
Arguments:
	...
	[optional] number - difficulty of the unit, only applies to "npc". Always 1 on 1.12.1; 1 by default.
