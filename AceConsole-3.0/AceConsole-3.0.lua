...
function Print(self, frame, arg)
	...
	if type(arg) == "string" then
		...
		local b, e = frame and 1 or 2, arg.n or tgetn(arg)
		if e >= b then
...
function AceConsole:Printf(a1, ...)
	...
	if type(a1) == "table" and a1.AddMessage then	-- Is first argument something with an .AddMessage member?
		frame, succ, s = a1, pcall(format, unpack(arg, 1, arg.n))
	else
		frame, succ, s = DEFAULT_CHAT_FRAME, pcall(format, a1, unpack(arg, 1, arg.n))
	end
