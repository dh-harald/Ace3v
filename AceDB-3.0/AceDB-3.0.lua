...
-- Vanilla: MINOR 26 fired with an argument count, Fire(event, argc, ...), and a registry may have been
-- built by a CallbackHandler with that Fire. Rebuild it, keeping the registrations.
local rebuildCallbacks = oldminor and oldminor < 27 and LibStub:GetLibrary("CallbackHandler-1.0", true)
for db in pairs(AceDB.db_registry) do
	local old = rawget(db, "callbacks")
	if rebuildCallbacks and old and old.events then
		db.callbacks = rebuildCallbacks:New(db)
		for event, handlers in pairs(old.events) do
			for owner, func in pairs(handlers) do db.callbacks.events[event][owner] = func end
		end
	end
