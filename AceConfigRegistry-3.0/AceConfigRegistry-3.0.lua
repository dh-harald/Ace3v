...
local AceConfigRegistry, oldminor = LibStub:NewLibrary(MAJOR, MINOR)
...
-- Vanilla: MINOR 16 fired with an argument count, Fire(event, argc, ...), and its registry may have been
-- built by a CallbackHandler with that Fire. Rebuild it, keeping the registrations.
if oldminor and oldminor < 17 and AceConfigRegistry.callbacks then
	local old = AceConfigRegistry.callbacks
	AceConfigRegistry.callbacks = CallbackHandler:New(AceConfigRegistry)
	for event, handlers in pairs(old.events) do
		for owner, func in pairs(handlers) do AceConfigRegistry.callbacks.events[event][owner] = func end
	end
end
