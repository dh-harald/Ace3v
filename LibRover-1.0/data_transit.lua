-- LibRover-1.0 portals, ships, zeppelins, the tram and class teleports for WoW 1.12.1.
--
-- Ported from ZygorGuidesViewerClassic's Libs-Classic/LibRover-1.0/data_transit.lua; the data itself is
-- unchanged.  Plain data in a global: the client may run a data file before the library's main
-- file, which adopts this table and clears the global.
...
---------------------
	...
	--## ALLIANCE ##--
		...
		-- Moonglade to Rut'theran Village Druid fake taxi
			"Moonglade 44.19,45.33 -to- Teldrassil 58.33,93.86 <region:ruttheran> {mode:PORTAL} {fac:A} {title:Take hippogryph to Rut'theran Village } {cost:145} {cond:RaceClassMatch('DRUID')}",

	--## HORDE ##--

		-- Moonglade to Thunderbluff Druid fake taxi
			"Moonglade 44.27,45.77 -to- Thunder Bluff 46.70,49.92 {mode:PORTAL} {fac:H} {title:Take windrider to Thunder Bluff} {cost:515} {cond:RaceClassMatch('DRUID')}",
