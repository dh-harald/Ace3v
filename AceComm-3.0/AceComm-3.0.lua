...
-- Vanilla: MINOR 9 fired with an argument count, Fire(prefix, argc, ...), and its registry may have been
-- built by a CallbackHandler with that Fire. Rebuild it, keeping the registrations.
if oldminor and oldminor < 10 and AceComm.callbacks then
	local old = AceComm.callbacks
	AceComm.callbacks = CallbackHandler:New(AceComm,
						"_RegisterComm",
						"UnregisterComm",
						"UnregisterAllComm")
	for event, handlers in pairs(old.events) do
		for owner, func in pairs(handlers) do AceComm.callbacks.events[event][owner] = func end
	end
end
